...
(
SELECT
city, MIN(size_m2) as min_size, MAX(size_m2) as max_size, CAST(AVG(build_year) AS INTEGER) as average_building_year,
CAST(AVG(price_sqm) AS INTEGER) AS average_price_sqm, COUNT(DISTINCT link) AS num_of_adverts, 
CAST(AVG(rent) AS INTEGER) AS average_rent
FROM 
...
SELECT 
city, market, CAST(AVG(price_sqm) AS INTEGER) AS average_price_sqm, 
CAST(AVG(size_m2) AS INTEGER) AS average_size_m2, CAST(AVG(build_year) AS INTEGER) AS average_build_year
FROM
(
SELECT DISTINCT
city,
...
CAST(AVG(price_sqm) as INTEGER) AS average_price_sqm, CAST(avg(rent) AS INTEGER) AS average_rent
FROM
(
SELECT DISTINCT
CASE
